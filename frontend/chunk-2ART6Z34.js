    ...
    summary.split(" | ").forEach((part) => {
      ...
      if (idx > 0) {
        ...
        if (key === "Job")
          ...
          p["confidence"] = isNaN(raw) ? val : raw > 1 ? raw + "%" : Math.round(raw * 100) + "%";
        } else if (key === "Raison")
          p["reason"] = val;
        else if (key === "ERREURS")
          p["errors"] = val;
    ...
    if (Array.isArray(raw)) {
      return raw.map((item) => {
        if (typeof item === "string") {
          const priorityMatch = item.match(/\[(HAUTE|MOYENNE|BASSE)\]/i);
          const priority = priorityMatch ? priorityMatch[1].toUpperCase() : null;
          const text = item.replace(/\[.*?\]/g, "").replace(/^\d+\.\s*/, "").trim();
          return { text, priority, done: false };
        }
        return {
          text: item.action || item.text || JSON.stringify(item),
...
//# sourceMappingURL=chunk-2ART6Z34.js.map
